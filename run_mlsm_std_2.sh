...
regionDivideSizes=(1 4 16 128) #MB
...
do
    ...
    do
        ...
        do
            ...
            do
                section=basic
                let rds=regionDivideSize*1024*1024
                ...
                if [ "$requestdistribution" = "zipfian" ]; then
                    ...
                    do
                        ...
                        if [ $run_type = 0 ]; then
                            ...
                            echo run mode
                            __runLSM "$outnameprefix"_run_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" "$changeRatio" RDS"$regionDivideSize"MB
                        else
                            echo load + run mode
                            __loadLSM "$outnameprefix"_load_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" "$changeRatio"
