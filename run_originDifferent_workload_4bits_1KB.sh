...
function __runLSM(){
	...
	runname="$old_runname"_t"$workload_type"
...
do
	...
	do
		...
		do
		    ...
		    __runLSM 0_5bufferedil03_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" 
		done
	    else
		...
		zipfianconst=0.99
		__runLSM 0_7bufferedil03_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" 
	    fi
