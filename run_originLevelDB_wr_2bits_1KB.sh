...
function __loadLSM(){
    ...
    ltype=$4
    bb=$5
    workloadw_name=./workloads/glsmworkloadw_"$levelIn"_"$value_size".spec
...
function __runLSM(){
    ...
    zipfian=$6
    cachesize=$7
    workloadr_name=./workloads/glsmworkloadwr_"$levelIn"_"$value_size".spec
	...
	sync;echo 1 > /proc/sys/vm/drop_caches
        python ./getStatistics.py "$runname"_"$j".txt /home/ming/statistics/experiment_"$experiment_time"_zipf"$zipfian"_cache"$cachesize"MB/
	if [ ! -d "$dirname" ]; then
	    mkdir  -p "$dirname"
	fi
...
do
	...
	do
	    ...
	    if [ "$requestdistribution" = "zipfian" ]; then
                echo "zipfian"
		dirname=/home/ming/experiment/lsm_"$DISK"_read_zipfian"$zipfianconst"/experiment"$experiment_time"_"$value_size"_wr/bloombits"$bloombits"level"$level"/open_files_"$maxOpenfiles"_allfound_1000WRead_directIO"$directIOFlag"_blockCacheSize"$blockCacheSize"MB_sizeRatio"$sizeRatio"
	    else
		echo "$requestdistribution"
		dirname=/home/ming/experiment/lsm_"$DISK"_read_"$requestdistribution"/experiment"$experiment_time"_"$value_size"/bloombits"$bloombits"level"$level"/open_files_"$maxOpenfiles"_allfound_1000WRead_directIO"$directIOFlag"_blockCacheSize"$blockCacheSize"MB_sizeRatio"$sizeRatio"
	    fi

	    #__loadLSM bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" 
            __runLSM bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits" "$zipfianconst" "$blockCacheSize"
